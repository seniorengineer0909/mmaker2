...
class TrendTrade {
  constructor(connection, walletList, tokenAddress, decimals) {
    ...
    this.emaStrategy = new emaStrategy_1.EmaStrategy(
      this.targetToken.address,
      this.configs.emaLongTerm,
      this.configs.emaShortTerm,
      this.configs.emaCalcDuring,
      this.configs.emaApiKey
    );
  ...
  // run EMA strategy
  run() {
    console.log("tokenAddress", this.targetToken.address)
  ...
  evaluateAndExecuteTrade() {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const solPrice = liqudityPoolData.priceUsd / (liqudityPoolData.priceNative || 1);
      let transaction;
      if (swapType == 'buy') {
        // console.log('!!!!!!!!!buy!!!!!!!!!');
        try {
          ...
          const swapAmountMax = new decimal_js_1.default(this.configs.swapAmountMaxEma)
            .dividedBy(liqudityPoolData.priceUsd)
            .mul(liqudityPoolData.priceNative)
            .mul(Math.pow(10, this.solToken.decimals));
          const swapAmountMin = new decimal_js_1.default(this.configs.swapAmountMinEma)
            .dividedBy(liqudityPoolData.priceUsd)
            .mul(liqudityPoolData.priceNative)
            .mul(Math.pow(10, this.solToken.decimals));
          let solAmountToSell = swapAmountMax
            .minus(swapAmountMin)
            .mul(Math.random())
            .plus(swapAmountMin)
            .floor();
          ...
          solAmountToSell = solAmountToSell
            ...
            .floor();
          //end add
          const { success, quote } = yield this.raydiumApi.getQuote(
          ...
          solVolume =
            new decimal_js_1.default(quote.inAmount)
          ...
          const wallet = yield this.selectWalletToBuy(Number(solAmountToSell.dividedBy(Math.pow(10, this.solToken.decimals)).add(0.1))); // add 0.1sol for sell and next buy fee
          // console.log({ wallet });
          if (wallet == null) {
  ...
  selectWalletToBuy(neededSolAmount) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      for (let i = 0; i < this.walletList.length; i++) {
        ...
        if (neededSolAmount <= balance) availableWalletList.push(this.walletList[i]);
        yield (0, sleep_1.sleep)(20);
      }
      if (maxBalance == 0) return null;
      if (availableWalletList.length == 0) return this.walletList[maxWalletNum];
      else return availableWalletList[Math.floor(Math.random() * availableWalletList.length)];
    });
  }
  selectWalletToSell() {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (maxBalance == 0) return { wallet: null, balance: 0 };
      else return { wallet: this.walletList[maxWalletNum], balance: maxBalance };
    });
  }
  ...
  getTokenBalance(walletAddress) {
    return __awaiter(this, void 0, void 0, function* () {
      try {
        const account = yield spl_token_1.Token.getAssociatedTokenAddress(
          spl_token_1.ASSOCIATED_TOKEN_PROGRAM_ID,
          spl_token_1.TOKEN_PROGRAM_ID,
          new web3_js_1.PublicKey(this.targetToken.address),
          walletAddress
        );
        const balance =
          Number((yield this.connection.getTokenAccountBalance(account)).value.amount) || 0;
        return balance;
      } catch (error) {
        return 0;
      }
